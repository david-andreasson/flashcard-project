...
a JSON body indicating the application and database status. This SHALL be the only
...
- **WHEN** `GET /actuator/health` is called without an `Authorization` header or session
